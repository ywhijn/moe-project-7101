...
class Qwen2MoeModel(Qwen2MoePreTrainedModel):
    ...
    def reset_logits(self):
        self.model.all_router_logits = ()
        for dec_layer in self.model.layers:
            if isinstance(dec_layer.mlp,Qwen2MoeSparseMoeBlock):
                dec_layer.mlp.router_logits=()
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            ############################## CHANGE
            if isinstance(decoder_layer.mlp, Qwen2MoeSparseMoeBlock):
                # print("in decoder layer",len(self.all_router_logits)," ",len(decoder_layer.mlp.router_logits))
                self.all_router_logits += (decoder_layer.mlp.router_logits)
